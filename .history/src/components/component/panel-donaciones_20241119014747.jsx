...
export function PanelDonacionesComponent() {
  ...
  const handleDeleteDonation = async () => {
    try {
      const token = localStorage.getItem('authToken')
      const res = await fetch(`http://localhost:3001/mercadopago/report/in-kind/${deletingDonation.donationId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="min-h-screen bg-[#ECE3D4] p-8">
              ...
              {donaciones.map((donacion, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-[#042637]">
                    ...
                    <Button onClick={() => setDeletingDonation(donacion)} className="bg-red-600 text-[#ECE3D4]">Eliminar</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      {deletingDonation && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded-lg shadow-lg">
            <h2 className="text-xl font-bold mb-4">Eliminar Donación</h2>
            <p className="mb-4">Por favor, ingrese la razón para eliminar esta donación:</p>
            <input
              type="text"
              value={deleteReason}
