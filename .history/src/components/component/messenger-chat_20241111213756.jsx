...
const MessengerChat = ({ receiverId, user, onCancel }) => {
  ...
  useEffect(() => {
    ...
    socket.emit('joinRoom', { userId: user._id, receiverId });
    ...
    return () => {
      socket.emit('leaveRoom', { userId: user._id, receiverId });
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await fetch(`http://localhost:3001/messages?receiverId=${receiverId}&senderId=${user._id}`);
        if (!res.ok) {
  ...
  const handleSend = () => {
    if (newMessage.trim()) {
      const message = {
        senderId: user._id, // Asegúrate de que user._id esté definido
        receiverId,
        text: newMessage,
      };
