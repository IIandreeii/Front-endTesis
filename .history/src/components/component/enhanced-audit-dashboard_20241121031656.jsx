...
export function EnhancedAuditDashboardComponent() {
  ...
  return (
    <div className="min-h-screen p-8" style={{ backgroundColor: '#ECE3D4' }}>
      <h1 className="text-4xl font-bold mb-8" style={{ color: '#042637' }}>Panel de Auditorías</h1>
      <div className="mb-4">
                  ...
                  {filteredData.map((audit) => {
                    ...
                    return (
                      <TableRow key={audit._id}>
                        ...
                        <TableCell>
                          {audit.action === 'edit' && audit.details && audit.details.before && audit.details.after && (
                            <span>
                              Valor cambiado de {audit.details.before.valuePerUnit} a {audit.details.after.valuePerUnit}
                            </span>
                          )}
                          {audit.action === 'report' && audit.details && (
                            <span>
                              {audit.details.reportType ? (
                                <>
                                  Tipo de reporte: {audit.details.reportType}, Donaciones: {audit.details.donationIds.length}
                                </>
                              ) : (
                                <>
                                  Periodo de reporte: {audit.details.period}
                                </>
                              )}
                            </span>
                          )}
                          {audit.action === 'delete' && audit.details && (
                            <span className="flex items-center space-x-2">
                              <AlertCircle className="h-4 w-4 text-red-500" />
                              <span>Razón: {audit.details.reason}</span>
                            </span>
                          )}
                          {audit.action === 'login' && audit.details && (
                            <span>
                              {audit.details.message}
                            </span>
                          )}
                          {audit.action === 'create' && audit.details && audit.details.donationData && (
                            <span>
                              Donación creada: {audit.details.donationData.itemType} - {audit.details.donationData.quantity} {audit.details.donationData.unit} a {audit.details.donationData.valuePerUnit} cada uno, donado por {audit.details.donationData.donorName}
                            </span>
                          )}
                        </TableCell>
                        <TableCell>
